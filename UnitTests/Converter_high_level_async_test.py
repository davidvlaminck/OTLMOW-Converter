...
from otlmow_converter.OtlmowConverter import OtlmowConverter, to_objects, to_file, to_dicts, to_dotnotation_dicts, \
    to_dataframe, to_dotnotation_dicts_async, to_dicts_async, to_objects_async, to_dataframe_async
...
async def test_dicts_to_dotnotation_dicts():
    ...
    new_list_of_ddicts = await to_dotnotation_dicts(orig_list_of_dicts, model_directory=model_directory_path)
    assert expected_list_of_ddicts == list(new_list_of_ddicts)
...
async def test_dotnotation_dicts_to_dicts():
    ...
    new_list_of_dicts = await to_dicts(orig_list_of_ddicts, model_directory=model_directory_path)
    assert expected_list_of_dicts == list(new_list_of_dicts)
...
async def test_dataframe_to_dataframe():
    ...
    created_df = await to_dataframe(expected_df, model_directory=model_directory_path)
    assert_frame_equal(expected_df, created_df)
...
async def test_dicts_to_dicts():
    ...
    new_list_of_dicts = await to_dicts(orig_list_of_dicts, model_directory=model_directory_path)
...
async def test_dotnotation_dicts_to_dotnotation_dicts():
    ...
    new_list_of_ddicts = await to_dotnotation_dicts(orig_list_of_ddicts, model_directory=model_directory_path)
...
async def test_dataframe_to_file():
    ...
    await OtlmowConverter.to_file(subject=df, file_path=excel_file_path, model_directory=model_directory_path)
    ...
    await OtlmowConverter.to_file(subject=df, file_path=json_file_path, model_directory=model_directory_path)
    ...
    await OtlmowConverter.to_file(subject=df, file_path=csv_file_path, model_directory=model_directory_path)
    ...
    await OtlmowConverter.to_file(subject=df, file_path=geojson_file_path, model_directory=model_directory_path)
...
async def test_objects_to_file():
    ...
    await OtlmowConverter.to_file_async(subject=orig_list_of_objects, file_path=json_file_path)
    from_json_objects = await OtlmowConverter.from_file_to_objects_async(
        json_file_path, model_directory=model_directory_path)
    assert orig_list_of_objects == list(from_json_objects)

    await OtlmowConverter.to_file_async(subject=orig_list_of_objects, file_path=csv_file_path)
    ...
    await OtlmowConverter.to_file_async(subject=orig_list_of_objects, file_path=geojson_file_path)
...
async def test_file_to_objects():
    ...
    await OtlmowConverter.from_objects_to_file_async(sequence_of_objects=orig_list_of_objects, file_path=csv_file_path)
    from_csv_objects_2 = await to_objects_async(csv_file_path_2, model_directory=model_directory_path)
    from_csv_objects_3 = await to_objects_async(csv_file_path_3, model_directory=model_directory_path)
    combined_list = list(from_csv_objects_2) + list(from_csv_objects_3)
    assert orig_list_of_objects == combined_list

    await OtlmowConverter.from_objects_to_file_async(sequence_of_objects=orig_list_of_objects, file_path=excel_file_path)
    from_excel_objects = await to_objects_async(excel_file_path, model_directory=model_directory_path)
    assert orig_list_of_objects == list(from_excel_objects)

    await OtlmowConverter.from_objects_to_file_async(sequence_of_objects=orig_list_of_objects, file_path=json_file_path)
    from_json_objects = await to_objects_async(json_file_path, model_directory=model_directory_path)
    assert orig_list_of_objects == list(from_json_objects)

    await OtlmowConverter.from_objects_to_file_async(sequence_of_objects=orig_list_of_objects, file_path=geojson_file_path)
    from_geojson_objects = await to_objects_async(geojson_file_path, model_directory=model_directory_path)
    assert orig_list_of_objects == list(from_geojson_objects)
...
async def test_dicts_to_objects():
    ...
    new_list_of_objects = await to_objects_async(sequence_of_dicts, model_directory=model_directory_path)
    assert orig_list_of_objects == list(new_list_of_objects)
