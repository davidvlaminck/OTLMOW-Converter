...
def test_export_relation():
    exporter = RDFExporter(settings={'waarde_shortcut': False})
    ...
    with pytest.warns(DeprecationWarning):
        relation = create_relation(source=instance, target=instance2, relation_type=Voedt)
...
def test_export_unnested_attributes(subtests):
    ...
    for expected_value, expected_type, attribute_uri in [
        ...
        with subtests.test(msg=f'testing single literal value: {attribute_uri}'):
            value = graph.value(subject=subj, predicate=URIRef(attribute_uri), any=False)
            assert value == Literal(expected_value, datatype=expected_type)

    with subtests.test(msg=f'testing single URI value: testKeuzelijst'):
        ...
        assert value == URIRef('https://wegenenverkeer.data.vlaanderen.be/id/concept/KlTestKeuzelijst/waarde-4')
    ...
    for expected_value, expected_type, attribute_uri in [
        ...
        attribute_uri = 'https://wegenenverkeer.data.vlaanderen.be/ns/onderdeel#AllCasesTestClass.' + attribute_uri
        with subtests.test(msg=f'testing multiple literal value: {attribute_uri}'):
            values = list(graph.objects(subject=subj, predicate=URIRef(attribute_uri)))
            for index, value in enumerate(values):
                assert value == Literal(expected_value[index], datatype=expected_type)

    with subtests.test(msg=f'testing multiple URI value: testKeuzelijstMetKard'):
...
def test_export_nested_attributes_level_1(subtests):
    ...
    for expected_values, uri1, uri2 in [
        ...
        with subtests.test(f'testing complex value level 1: {uri1}'):
            ...
            for index, row in enumerate(q_res):
                assert row.o == expected_values[index]
...
def test_export_nested_attributes_level_higher(subtests):
    ...
    for expected_values, uri1, uri2, uri3 in [
        ...
        with subtests.test(msg=f'testing complex values level 2: {uri1}'):
    ...
    for expected_values, uri1, uri2, uri3, uri4 in [
     ...
     'https://wegenenverkeer.data.vlaanderen.be/ns/implementatieelement#DtcTestComplexType2.testKwantWrd',
        ...
        with subtests.test(msg=f'testing complex values level 3: {uri1}'):
            query = """SELECT ?o WHERE { ?s ?p1 ?c1 .
                                 ?c1 ?p2 ?c2 .
                                 ?c2 ?p3 ?c3 .
                                 ?c3 ?p4 ?o .}"""
